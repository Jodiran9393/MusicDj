...
function Playlist({ allTracks, onSelectTrack, currentTrackId }) {
  ...
  // Drag and drop handlers for playlist tracks
  const onTrackDragStart = idx => setDraggedTrackIdx(idx);
  const onTrackDragOver = idx => e => {
    e.preventDefault();
    if (draggedTrackIdx === null || draggedTrackIdx === idx) return;
    const tracks = [...playlistTracks];
    const [removed] = tracks.splice(draggedTrackIdx, 1);
    tracks.splice(idx, 0, removed);
    // Update the playlist order
    setPlaylists(pls => pls.map(pl => pl.name === selectedPlaylist ? { ...pl, tracks } : pl));
    setDraggedTrackIdx(idx);
  };
  const onTrackDragEnd = () => setDraggedTrackIdx(null);
  ...
  return (
    <div style={{ maxWidth: 400, margin: '0 auto', marginTop: 24, background: '#f5f7fa', borderRadius: 10, boxShadow: '0 2px 8px #e0e0e0', padding: 20 }}>
      <h2 style={{ color: 'var(--text-color)', marginBottom: 16, marginTop: 8, letterSpacing: 1, textShadow: '0 1px 0 #fff, 0 2px 8px #bbb', fontWeight: 700, fontSize: 20 }}>Playlist</h2>
      ...
      {selectedPlaylist && (
        <div style={{ marginTop: 22 }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            ...
            {playlistTracks.map((trackId, idx) => {
              const track = allTracks.find(t => t.id === trackId);
              if (!track) return null;
              return (
                <li
                  key={trackId}
                  style={{ marginBottom: 5, display: 'flex', alignItems: 'center', gap: 6, opacity: draggedTrackIdx === idx ? 0.6 : 1, background: draggedTrackIdx === idx ? '#f0f0f0' : undefined }}
                  draggable
                  onDragStart={() => onTrackDragStart(idx)}
                  onDragOver={onTrackDragOver(idx)}
                  onDragEnd={onTrackDragEnd}
                  onDrop={onTrackDragEnd}
                >
                  <span style={{ cursor: 'grab', fontSize: 18, marginRight: 6 }}>⋮</span>
